...
% The purpose of this edit is to allow this program to be called as a
...
function [points, endurance, autocross] = Lap_Sim_CSAPS_Func(varName,varValue)
    ...
    arguments (Repeating)
         varName (1,1) string {mustBeValidVariableName}
         varValue (1,1) double {mustBeNumeric}
    end

    %creates a variable named varName, with the value of varValue
    for i = 1:nargin/2
        expression = append(varName{i},'=',num2str(varValue{i}),';');
        eval(expression);
    end

    % all future variables are edited to allow variables defined in function
    % call to take precedence, This does not affect variables defined from
    % files

    % currently cannot define engine speed, engine torque, and transmission
    % gear ratios because only single variable can be defined, but these
    % values are probably mostly defined so it should be fine

    %this function includes the changes I made to the original lap sim to
